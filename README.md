...
# Efficacy tests
- Testing with `np.optimization.linprog` (with both `highs` and `simplex` methods), the `min_example4.txt` on our program gives the same target optimal value and the same optimal solution (no feasible optimal solution). However, the model in this example is poorly stated, there should be no equality $x_1+x_2+x_3+x_4 = 100$.
